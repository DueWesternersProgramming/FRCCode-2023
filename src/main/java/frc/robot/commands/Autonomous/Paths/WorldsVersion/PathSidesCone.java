...
public class PathSidesCone extends SequentialCommandGroup {
     ...
     */
    public PathSidesCone(DriveSubsystem m_drive, ArmSubsystem m_arm, IntakeSubsystem m_intake, WristSubsystem m_wrist, LightSubsystem m_light, VisionSubsystem m_vision) {
        addCommands(
            new CalibrateGyro(m_drive),

            new ParallelCommandGroup(new IntakeOn(m_intake), new WristUnlatch(m_wrist), new SetVisionPipeline(m_vision, 1), new SequentialCommandGroup(new setBrake(m_drive, m_light), new AutoSpeed(m_drive))),
            ...
            new ParallelCommandGroup(new IntakeOff(m_intake), new WristIn(m_wrist), new ArmRetract(m_arm), new DriveDistance(m_drive, -46, 0.1)),
            //new WaitCommand(0.5),
